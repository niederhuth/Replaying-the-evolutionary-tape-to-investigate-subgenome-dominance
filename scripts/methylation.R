...
controls <- c("TO1000","IMB218","combined2")
#plots <- c('all_LTRs')
...
for(x in plots){
  ...
  for(y in c('CG','CHG')){
    ...
    ggsave(paste(x,y,"_metaplot.pdf1",sep="_"),plot,path="plots/")
  }
}
...
for(x in plots){
  for(j in line <- unique(samples$Line)){
    ...
    for(y in c('CG','CHG')){
      ...
      ggsave(paste(x,j,y,"_metaplot.pdf1",sep="_"),plot,path="plots/")
    }
  }
}
...
for(x in c('combined','R500','TO1000')){
  ...
  ggsave(paste(x,"total_methylation.pdf1",sep="_"),plot,path="plots/")
  
  for(y in c('CG','CHG','CHH')){
    df2 <- df1[df1$Var2==y,]
    plot <- ggplot(df2,aes(x=reorder(order,generation2),y=value,fill=generation)) + 
      geom_bar(stat="identity",position="dodge") +
      theme(panel.background=element_blank(),
            panel.grid=element_blank(),
            axis.text.y=element_text(color="black"),
      ...
      xlab("") +
      scale_y_continuous(expand=c(0,0), labels=percent) +
      scale_x_discrete(labels=df2$sample)
    #scale_fill_manual("",values=c("tomato3","orange3","magenta3","royalblue3"))
    ggsave(paste(x,y,"_total_methylation.pdf1",sep="_"),plot,path="plots/")
  }
}
...
for(j in line <- unique(samples$Line)){
  for(x in c('combined','R500','TO1000')){
    df1 <- matrix(nrow=0,ncol=0)
    for(i in c(as.vector(samples[samples$Line=="EL200",]$Sample),controls)){
      input=paste(x,paste(i,x,"weighted_methylation.txt",sep="_"),sep="/")
      df2 <- t(read.table(input,header=T,sep="\t")[6])
      colnames(df2) <- c('CG','CHG','CHH')
      df2 <- melt(df2)
    ...
    for(y in c('CG','CHG','CHH')){
      ...
      plot <- ggplot(df2,aes(x=reorder(order,generation2),y=value,fill=generation)) + 
        ...
        scale_x_discrete(labels=df2$sample)
      #scale_fill_manual("",values=c("tomato3","orange3","magenta3","royalblue3"))
      ggsave(paste(j,x,y,"_total_methylation.pdf1",sep="_"),plot,path="plots/")
    }
  }
}
